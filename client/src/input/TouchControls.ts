...
export class TouchControls {
  ...
  constructor(look: LookSink) {
    ...
    this.root.className = 'obby-touch';
    ...
    this.stick.className = 'obby-touch__stick';
    this.knob = document.createElement('div');
    this.knob.className = 'obby-touch__knob';
  ...
  setSuppressed(suppressed: boolean): void {
    ...
    this.root.classList.toggle('obby-touch--hidden', suppressed);
  ...
  private readonly onCanvasDown = (event: PointerEvent): void => {
    ...
    if (this.movePointer === null && this.inStickZone(event.clientX, event.clientY)) {
      ...
      this.placeStick();
      this.stick.classList.add('obby-touch__stick--active');
  ...
  private readonly onJumpDown = (event: PointerEvent): void => {
    ...
    this.jumpHeld = true;
    this.jumpPulse = true;
    this.jumpButton.classList.add('obby-touch__jump--down');
  ...
  private readonly onJumpUp = (event: PointerEvent): void => {
    event.stopPropagation();
    this.jumpHeld = false;
    this.jumpButton.classList.remove('obby-touch__jump--down');
    try {
  ...
  private releaseStick(): void {
    ...
    this.sprint = false;
    this.knob.style.transform = 'translate(-50%, -50%)';
    this.stick.classList.remove('obby-touch__stick--active');
...
.obby-touch[hidden] { display: none; }
...
.obby-touch__stick {
  ...
  height: calc(var(--obby-stick-radius) * 2);
  margin: calc(var(--obby-stick-radius) * -1) 0 0 calc(var(--obby-stick-radius) * -1);
...
.obby-touch__jump {
  position: fixed;
  right: calc(var(--obby-safe-r, 0px) + 24px);
  bottom: calc(var(--obby-safe-b, 0px) + 34px);
...
.obby-touch__jump--down { transform: translateY(4px); box-shadow: none; filter: brightness(1.12); }
`;
