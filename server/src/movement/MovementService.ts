 ...
 * at once and sprint across the map. 1.5 leaves generous room for jitter and
 * frame-rate spikes while making sustained speed-hacking impossible.
 */
 ...
 * backflip state. Because the very same `stepPlayer` runs on the client for
 ...
 * is going, whether it may jump or flip - is decided here from state the
 * client cannot reach.
 */
export class MovementService {
  ...
  initialise(player: PlayerState): void {
    ...
    motion.backflipsRemaining = player.backflipCapacity;
    this.sims.set(player.sessionId, {
      motion,
    ...
    this.publish(player, this.sims.get(player.sessionId) as Sim);
  }
   ...
   *
   * Progression earned by running ON THE SPOT has no position delta to measure,
   * so it is paid per simulated second instead - which has to be the server's
   * own step, never a duration the client asserts.
   */
  get lastStep(): number {
  ...
  teleport(
    ...
    if (!sim) return;
    resetMotion(sim.motion, player.backflipCapacity, x, y, z, yaw);
  ...
  applyInput(sessionId: string, player: PlayerState, message: MoveMessage): boolean {
    ...
    stepPlayer(
      ...
      sanitiseInput(message),
      {
        ...
        backflipCapacity: player.backflipCapacity,
        // Resolved from the SERVER's rebirth count, never from the message.
        maxTreadmillTier: maxUsableTreadmill(player.rebirths),
      },
      step,
      this.collision,
      sim.events,
  ...
  /** Keep the replicated allowance in step with the player's level. */
  syncCapacity(sessionId: string, player: PlayerState): void {
    const sim = this.sims.get(sessionId);
    if (!sim) return;
    if (sim.motion.grounded) {
      sim.motion.backflipsRemaining = player.backflipCapacity;
    }
  }
  ...
  private publish(player: PlayerState, sim: Sim): void {
    ...
    // `speed` is the ANIMATION signal, not physics - the authoritative
    // velocity is replicated separately and stays zero on a treadmill. A
    // runner is travelling nowhere but is very much running, so report the
    // speed they are running AT, or remote clients would show them idling on
    // the spot.
    player.speed =
      m.treadmillTier > 0
        ? MOVEMENT.runSpeed * player.moveMultiplier
        : horizontalSpeed(m);
    ...
    player.treadmillTier = m.treadmillTier;
  }
